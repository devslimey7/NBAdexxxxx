...
class Pack(models.Model):
    ...
    emoji = fields.CharField(max_length=20, description="Emoji for the pack", null=True)
    cost = fields.IntField(description="Cost in coins to purchase this pack")
    description = fields.TextField(description="Description of pack contents", null=True, default="")
...
class CoinReward(models.Model):
    ...
    pack: fields.ForeignKeyRelation[Pack] = fields.ForeignKeyField(
        "models.Pack", related_name="rewards", on_delete=fields.CASCADE
    )
    amount = fields.IntField(description="Amount of coins rewarded")
    probability = fields.FloatField(default=1.0, description="Probability of getting this reward (0-1)")
    description = fields.CharField(max_length=128, description="Description of this reward", null=True)
...
class CoinTransaction(models.Model):
    ...
    id: int
    player: fields.ForeignKeyRelation[Player] = fields.ForeignKeyField(
        "models.Player", related_name="coin_transactions", on_delete=fields.CASCADE
    )
    amount = fields.IntField(description="Amount of coins (positive for gain, negative for loss)")
    reason = fields.CharField(max_length=128, description="Reason for transaction")
    pack: fields.ForeignKeyRelation[Pack] | None = fields.ForeignKeyField(
        "models.Pack", null=True, default=None, on_delete=fields.SET_NULL
    )
    created_at = fields.DatetimeField(auto_now_add=True)
    ...
    class Meta:
        indexes = [
            PostgreSQLIndex(fields=("player_id",)),
            PostgreSQLIndex(fields=("created_at",)),
        ]
